...
from src.GCN import GraphConvolution
from scipy import io as scio
import os
from torch.utils.data import DataLoader
from src.VTR import VTRnet
...
class MULTModel(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.gcn_layer = GraphConvolution(self.in_features, self.out_features)
        self.vtr = VTRnet(2048, 2048, 2048)
        self.lam1 = float(self.config['lam1'])
        self.lam2 = float(self.config['lam2'])
    ...
    def forward(self, x_v, x_a, x_t):
        ...
        """
        #
        vc, ac, tc, vp, ap, tp, vd, ad, td = self.vtr(x_v, x_a, x_t)
        recon_loss = (F.mse_loss(vd, x_v) + F.mse_loss(td, x_t) + F.mse_loss(ad, x_a)) / 3
        orth_loss = torch.norm(torch.mm(vp.T, vc)) + torch.norm(torch.mm(tp.T, tc)) + torch.norm(torch.mm(ap.T, ac))

        #x_a = torch.as_tensor(x_a)
        x_a = x_a.unsqueeze(dim=1)
        #x_a = x_a.transpose(1, 2)
        # (23,1,2048)
        #x_aa = x_a.transpose(0, 1)[0]

        #x_v = torch.as_tensor(x_v)
        x_v = x_v.unsqueeze(dim=1)
        #x_v = x_v.transpose(1, 2)
       # x_vv = x_v.transpose(0, 1)[0]

        #x_t = torch.as_tensor(x_t)
        x_t = x_t.unsqueeze(dim=1)
        #x_t = x_t.transpose(1, 2)
        #x_tt = x_t.transpose(0, 1)[0]
        ...
        #
        # h_v_with_ts = self.trans_v_with_t(x_t, x_a, x_a)
        # h_v_with_as = self.trans_v_with_a(x_v, x_a, x_a)
        # h_v_with_vs = self.trans_v_with_v(x_a, x_a, x_a)
        ...
        last_h_vat = h_v_with_ats[0]  # wanbei（23， 2048）
        ...
        # # (23,2048)
        # # loss_vt = self.criterionCycle(h_v_with_ats, x_tt)
        # # loss_va = self.criterionCycle(h_v_with_ats, x_aa)
        # # loss_vv = self.criterionCycle(h_v_with_ats, x_vv)
        # # loss_1_total = loss_vt + loss_va + loss_vv
        #
        #
        # loss_G_cycle = self.criterionCycle(output_vv, last_h_vat) * self.v + self.criterionCycle(output_va,
        #                                                                                          last_h_vat) * self.a + self.criterionCycle(
        #     output_vt, last_h_vat) * self.t
        # loss_total = self.D * loss_D
        inp = torch.Tensor(scio.loadmat('/media/HardDisk/lsy/meipai_dataset/LabelVectorized.mat')['labelVector'])
        adj = torch.Tensor(scio.loadmat('/media/HardDisk/lsy/meipai_dataset/shuffle_data/adj_less.mat')['adj_less'])
        adj = self.gcn_layer(inp, adj)
        ...
        output = torch.matmul(output, adj)
        loss_total = self.lam1*recon_loss + self.lam2*orth_loss
