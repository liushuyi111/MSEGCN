...
class VTRnet(nn.Module):
    ...
    def __init__(self, dim_vision_encoder, dim_text_encoder, dim_tract_encoder, dropout_rate=0.5):
        super(VTRnet, self).__init__()

        self.vision_dim = dim_vision_encoder  ##2048
        self.text_dim = dim_text_encoder  ## 2048
        self.tract_dim = dim_tract_encoder ## 2048
        self.hidden = 1024
        ...
        ##########################################
        # private tract encoder
        ##########################################
        self.tract_encoder = nn.Sequential(
            nn.Linear(self.vision_dim, self.hidden),
            nn.BatchNorm1d(self.hidden),
            nn.Dropout(dropout_rate),
            nn.ReLU(),
        ...
        self.tract_private_encoder = nn.Sequential(
            nn.Linear(self.hidden, 786), nn.BatchNorm1d(786),
            nn.Dropout(dropout_rate), nn.ReLU())
        ...
        self.tract_decoder = nn.Sequential(
            # nn.Linear(786, (786+self.text_dim)//2),
            # nn.BatchNorm1d((786 + self.text_dim) // 2),
            # nn.Dropout(dropout_rate),
            # nn.Tanh(),
    ...
    def forward(self, vision, text, tract): #text换成audio
        ...
        tractEncoder = self.tract_encoder(tract)
        tract_private_encoder = self.tract_private_encoder(tractEncoder)  # h^p_t
        tract_common_encoder = self.common_encoder(textEncoder)

        tract_decoder = self.tract_decoder(tract_private_encoder +
                                         tract_common_encoder)

        # VT = self.VT_Linear(torch.cat((vision_private_encoder, text_private_encoder), dim=1))
        #encoder(23, 786)
        #decoder(23, 2048)

        return vision_common_encoder, text_common_encoder, tract_common_encoder, vision_private_encoder, text_private_encoder, tract_private_encoder, vision_decoder, text_decoder,tract_decoder

if __name__ == '__main__':
    D = torch.randn(23, 2048)
    B = torch.randn(23, 2048)
    C = torch.randn(23, 2048)
    net = VTRnet(2048, 2048, 2048)
    out = net(D,B,C)
    print(out)